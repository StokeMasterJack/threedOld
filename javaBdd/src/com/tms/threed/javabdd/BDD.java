 ...
 *
 * @see com.tms.threed.javabdd.BDDFactory
 * @see com.tms.threed.javabdd.BDDDomain#set()
 *
 * @author John Whaley
 * @version $Id$
 */
public abstract class BDD implements Serializable {
     ...
     * @see com.tms.threed.javabdd.BDD#simplify(BDD)
     */
...
//    }
    ...
    */
     ...
     */
    public static class BDDIterator implements Iterator<boolean[]> {
        final BDDFactory f;
        final AllSatIterator allSatIterator;
        ...
        final boolean[] currentBitAssignment;

        // Latest result from allsat iterator.
        byte[] latestAllSatResult;
         ...
         * @param varSet  variable set to mention in result
         */
        public BDDIterator(BDD bdd, BDDVarSet varSet) {
            initialBDD = bdd;
            f = bdd.getFactory();
            allSatIterator = new AllSatIterator(bdd, true);
            // init v[]
            varLevels = varSet.toLevelArray();
            // init b[]
            currentBitAssignment = new boolean[varLevels.length];
            gotoNext();
        }

        protected void gotoNext() {
            if (allSatIterator.hasNext()) {
                latestAllSatResult = (byte[]) allSatIterator.next();
            } else {
                latestAllSatResult = null;
                return;
            }
            for (int i = 0; i < varLevels.length; ++i) {
                int vi = varLevels[i];
                if (latestAllSatResult[vi] == 1) currentBitAssignment[i] = true;
                else currentBitAssignment[i] = false;
            }
        }

        protected boolean gotoNextA() {
            for (int i = varLevels.length - 1; i >= 0; --i) {
                int vi = varLevels[i];
                if (latestAllSatResult[vi] != -1) continue;
                if (currentBitAssignment[i] == false) {
                    currentBitAssignment[i] = true;
                    return true;
                }
                currentBitAssignment[i] = false;
            }
            return false;
        }

        public boolean hasNext() {
            return latestAllSatResult != null;
        }

        @Override public boolean[] next() {
            //DF: nextBDD() is *really* slow.
            //So i replaced it with a call to nextSat() instead
            //return nextBDD();
            return nextSat();
        }

//        public BigInteger nextValue(BDDDomain dom) {
//            if (latestAllSatResult == null) {
//                throw new NoSuchElementException();
//            }
//            lastReturned = null;
//            BigInteger val = BigInteger.ZERO;
...
//                int k = Arrays.binarySearch(varLevels, level);
//                if (k < 0) {
//                    val = null;
//                    break;
//                }
//                if (currentBitAssignment[k]) {
//                    val = val.add(BigInteger.ONE);
//                }
//            }
//            if (!gotoNextA()) {
...
//        }
        ...
        public boolean[] nextSat() {
            ...
            for (int i = 0; i < currentBitAssignment.length; ++i) {
                result[f.level2Var(varLevels[i])] = currentBitAssignment[i];
            }
            if (!gotoNextA()) {
                gotoNext();
            }
        ...
        public BDD nextBDD() {
            ...
            for (int i = varLevels.length - 1; i >= 0; --i) {
                ...
                if (currentBitAssignment[i] == true) lastReturned.andWith(f.ithVar(vi));
...
//            lastReturned = null;
        ...
        public boolean isDontCare(int var) {
            if (latestAllSatResult == null) return false;
            int level = f.var2Level(var);
            return latestAllSatResult[level] == -1;
        }
        ...
        public void fastForward(int var) {
            ...
            int level = f.var2Level(var);
            int i = Arrays.binarySearch(varLevels, level);
            if (i < 0 || latestAllSatResult[i] != -1)
                throw new BDDException();
            currentBitAssignment[i] = true;
        }
...
//    }
     ...
     *
     * @see com.tms.threed.javabdd.BDD.BDDToString
     *
